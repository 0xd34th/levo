...
describe('SevenKSwapPanel', () => {
  ...
  it('disables quoting outside mainnet with clear copy', async () => {
    ...
    expect(buttonByText(host, 'Get quote')).toHaveProperty('disabled', true);
  });

  it('disables quote for invalid amount, missing wallet, or same-token pair', async () => {
    useEmbeddedWalletMock.mockReturnValueOnce({
      suiAddress: null,
      loading: false,
      error: null,
    });
  ...
  it('renders quote review fields from the swap quote response', async () => {
    ...
    await act(async () => {
      buttonByText(host, 'Get quote').click();
    });
