...
// POST /api/v1/agent/mandate/[id]/revoke
//
// 2-step Privy flow. After revoke confirms, the mandate is REVOKED in DB and
// any future Seal seal_approve dry-run against the on-chain mandate will fail.
export async function POST(
  req: NextRequest,
  { params }: { params: Promise<{ id: string }> },
) {
  const ip = getClientIp(req);
  const rl = await rateLimit(`agent-mandate-revoke:${ip}`, 60, 10);
  if (!rl.allowed) {
  ...
  try {
    ...
    return noStoreJson(
      { error: error instanceof Error ? error.message : 'Mandate revoke failed' },
      { status: 400 },
    );
  }
}
