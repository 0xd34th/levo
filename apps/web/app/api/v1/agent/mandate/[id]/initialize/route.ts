...
// POST /api/v1/agent/mandate/[id]/initialize
//
// 2-step Privy flow (matches /mandate/create). Sets the mandate's on-chain
// witness_commit to step[0].currentCommit from the pre-generated chain.
// Per F-5, only the owner can call set_initial_witness.
export async function POST(
  req: NextRequest,
  { params }: { params: Promise<{ id: string }> },
) {
  const ip = getClientIp(req);
  const rl = await rateLimit(`agent-mandate-init:${ip}`, 60, 10);
  if (!rl.allowed) {
  ...
  try {
    ...
    return noStoreJson(
      { error: error instanceof Error ? error.message : 'Mandate initialize failed' },
      { status: 400 },
    );
  }
}
