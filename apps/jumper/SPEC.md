...
修复 `jumper.krilly.ai` 首页因可选营销/广告配置缺失而进入全局错误页的问题，并确保主交易流在缺少这些配置时仍可正常渲染。
...
4. 未注入 GA 脚本时，`useUserTracking()` 的 GA 调用不会抛运行时错误。
5. 相关单测通过，且 `apps/jumper` 的 `test:unit`、`typecheck`、`build` 通过。
