...
import { getSiteUrl, JUMPER_URL } from '@/const/urls';
...
async function getLoyaltyPassData(
  ...
  const res = await fetch(`${apiBaseUrl}/wallets/${walletAddress}/rewards`, {
    headers: {
      ...
      referer: process.env.NEXT_PUBLIC_SITE_URL ?? JUMPER_URL,
    },
  });
...
export async function GET(
  ...
  try {
    ...
    const loyaltyPass = await getLoyaltyPassData(validatedAddress);
