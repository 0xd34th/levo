...
- 9 个数据工具 + 2 个写入工具：
  ...
  - `prepare_swap` / `SwapCard`（锁定用户输入金额，客户端实时取 7K MetaAg 报价并签名）
...
- swap 不走服务端报价/构建；`prepare_swap` 解析 token metadata 和 raw amount，`SwapCard` 在客户端用 7K MetaAg 实时报价、路由、slippage 和签名。
